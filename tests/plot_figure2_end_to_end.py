#!/usr/bin/env python3
"""Dependency-free, paper-facing SVG plots for Figure 2."""
...
ALGORITHM_STYLES = {
    "xyz_sketch": {"label": "XYZ-Sketch", "color": "#0057ff", "dash": "", "marker": "circle"},
...
def write_source_summary(path: Path, input_path: Path, rows: list[dict[str, Any]], include_unresolved: bool) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("- Figure 2(a): log-log axes; R_w30 = transmitted bits / (30*d).\n")
        handle.write("- Figure 2(b,c): log-log axes. Zero timing values are treated as unavailable, not as zero cost.\n")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot paper Figure 2 from compare-frontier CSV or JSONL output.")
    parser.add_argument("--input", type=Path, default=Path("tests") / "results" / "paper_fig2_end_to_end" / "summary.csv")
...
def main() -> None:
    ...
    plots = [
        ("best_R_w30", "figure2a_space.svg", "Space overhead", "space overhead R"),
        ("update_avg_s_per_element", "figure2b_update_cost.svg", "Update time", "update time per input element"),
        ("decode_avg_s_per_difference", "figure2c_decode_cost.svg", "Decode time", "decode time per difference"),
    ]
